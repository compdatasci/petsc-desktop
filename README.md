# Docker Image for PETSc
This Docker image provides the Ubuntu 17.10 environment with X Windows with PETSc. The X Windows will display in your web browser in full-screen mode. You can use this Docker image on 64-bit Linux, Mac or Windows. It allows you to use the same programming environment regardless which OS you are running on your laptop or desktop.

[![Build Status](https://travis-ci.org/compdatasci/petsc-desktop.svg?branch=master)](https://travis-ci.org/compdatasci/petsc-desktop) [![](https://images.microbadger.com/badges/image/compdatasci/petsc-desktop.svg)](https://microbadger.com/images/compdatasci/petsc-desktop)
...
To run the Docker image, first download the script [`petsc_desktop.py`](https://raw.githubusercontent.com/compdatasci/petsc-desktop/master/petsc_desktop.py)
...
```
curl https://raw.githubusercontent.com/compdatasci/petsc-desktop/master/petsc_desktop.py -outfile petsc_desktop.py
```
On Linux or Mac, start a terminal, use the `cd` command to change to the working directory, and then run the following command:
```
curl -s -O https://raw.githubusercontent.com/compdatasci/petsc-desktop/master/petsc_desktop.py
```

After downloading the script, you can start the Docker image using the command
```
python petsc_desktop.py -p
```
...
```
python petsc_desktop.py -v myproject
```
and the work directory will be the data volume.

For additional command-line options, use the command
```
python petsc_desktop.py -h
```
...
default web browser on your computer. Simply replace `petsc_desktop.py` with `petsc_jupyter.py` in the preceding commands. That is, on Windows run the commands
```
curl https://raw.githubusercontent.com/compdatasci/petsc-desktop/master/petsc_jupyter.py -outfile petsc_jupyter.py
python petsc_jupyter.py -p
```
or on Linux and Mac run the commands
```
curl -s -O https://raw.githubusercontent.com/compdatasci/petsc-desktop/master/petsc_jupyter.py
python petsc_jupyter.py -p
```
...
```
python petsc_desktop.py
```
or
```
python petsc_jupyter.py
```
